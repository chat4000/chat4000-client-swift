...
struct QRScannerView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                VStack(spacing: 12) {
                    Text("Point at a chat94 QR code")
                        .font(AppFonts.caption)
                        .foregroundStyle(.white)
    ...
    private func handleScannedCode(_ code: String) {
        ...
        guard normalized.count == 8 || pairingInvite != nil || parsedConfig != nil else {
            errorMessage = "Not a valid chat94 pairing code or group key"
            Task {
                try? await Task.sleep(for: .seconds(2))
                if errorMessage == "Not a valid chat94 pairing code or group key" {
                    errorMessage = nil
                }
            }
            return
...
final class QRCameraPlatformView: PlatformView, @preconcurrency AVCaptureMetadataOutputObjectsDelegate {
    ...
    private func configureCameraIfNeeded() {
        ...
        output.setSampleBufferDelegate(self, queue: DispatchQueue(label: "chat94Mac.QRFrames"))
        captureSession.addOutput(output)
        #endif
