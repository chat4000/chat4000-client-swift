...
final class PlatformAppDelegate: NSObject, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        AppLog.log("🔔 [push] didRegisterForRemoteNotifications delegate fired")
        PushNotificationManager.shared.storeDeviceToken(deviceToken)
    }

    func application(_ application: UIApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
        AppLog.log("⚠️ [push] remote notification registration failed: \(error.localizedDescription)")
    }
    ...
    func application(
        ...
        AppLog.log("🔔 [push] app delegate didReceiveRemoteNotification invoked")
...
final class PlatformAppDelegate: NSObject, NSApplicationDelegate {
    func applicationDidFinishLaunching(_ notification: Notification) {
        AppLog.log("🚀 [push] mac app didFinishLaunching")
    }

    func application(_ application: NSApplication, didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data) {
        AppLog.log("🔔 [push] mac didRegisterForRemoteNotifications delegate fired")
        PushNotificationManager.shared.storeDeviceToken(deviceToken)
    }

    func application(_ application: NSApplication, didFailToRegisterForRemoteNotificationsWithError error: Error) {
        AppLog.log("⚠️ [push] mac remote notification registration failed: \(error.localizedDescription)")
    }

    func application(_ application: NSApplication, didReceiveRemoteNotification userInfo: [String: Any]) {
        AppLog.log("🔔 [push] mac app delegate didReceiveRemoteNotification invoked")
        Task { @MainActor in
