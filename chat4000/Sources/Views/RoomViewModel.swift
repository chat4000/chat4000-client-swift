...
final class RoomViewModel {
    ...
    // The chunk-scoped dup-prefetch index was REMOVED (2026-07-13): its
    // `Collection.contains` #Predicate compiled but threw an uncatchable ObjC
    // NSException inside CoreData at fetch time (three SIGABRTs, each ~1s after
    // the first chunk of a launch). Dup checks are back to per-event indexed
    // fetchLimit-1 lookups — with chunked frames and status collapse the volume
    // is small, and correctness beats the saved queries.
